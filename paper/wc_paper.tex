...
\maketitle
\begin{abstract}
\noindent Ehrmann and Jansen show that trades on a country's national exchange roughly halve when its team plays in the FIFA World Cup, carried by a local trader who is also the viewer. Two questions sit conflated on that mechanism: whether the global attention shock moves aggregate trading in the continuous, global, derivative venues where no team maps to the marginal trader, and whether the local effect generalizes beyond its first exchange. We separate them.

\smallskip\noindent Our most transferable result is that two natural ways of measuring the effect manufacture it from no underlying response: a cross-market comovement decline ($-0.086$, $p=0.005$) that vanishes once the set of open instruments is held fixed, and an all-session cash-equity volume increase ($+19\%$) that is extended-hours sampling --- each reproducing the prior finding from a process with no match effect, the comovement case shown general by a Monte Carlo.

\smallskip\noindent On one-minute data for eleven instruments across five market structures over 2018 and 2022, the aggregate channel is bounded near zero: equivalence tests reject a $30\%$ effect in every group (declines beyond about $9\%$ excluded in crypto spot, $6\%$ pooled), and an injected $30$--$45\%$ decline is recovered with full power, so these are genuine nulls, not weak tests. The local channel we bound rather than refute. No mapped venue we can reach shows a decline and the daily home response is bounded below about $11\%$; but the exact Ehrmann--Jansen object --- a minute-level own-exchange trade count whose trader is the viewer --- is left untested for want of free foreign intraday data. The next-day loser effect is absent at daily frequency in 20 national indices and 16 currencies.

\smallskip\noindent We do not overturn the distraction effect in its own setting; the contribution is the bound on the aggregate channel, the evidence that it does not travel to the venues that now carry most trading, and the two mechanisms that show how readily it is mismeasured.
...
Wikimedia hourly pageview dumps settle it at the during-match resolution, and we check ten matches spanning
every round from group stage to final across both tournaments rather than rest the point on one.
Reconstructing each match hour by hour, the two competing national-team articles and the tournament article
jump in the kickoff hour against the same articles three hours earlier. For the 2022 final
(Argentina--France, kickoff $15{:}00$ UTC, 2022-12-18) the tournament article runs $2.9\times$
($5{,}245\to15{,}219$ views), Argentina's $3.3\times$ ($1{,}038\to3{,}399$), France's $3.1\times$
($893\to2{,}775$), and the Lionel Messi article $3.8\times$ ($1{,}984\to7{,}582$). The pattern holds across
all ten: the team-article kickoff-hour spike averages $3.4\times$ the pre-match baseline, with per-match
means from $1.7$ to $5.7\times$ and a clear rise in every match. The largest jumps are the group-stage
surprises (Argentina--Saudi Arabia $5.0\times$, Germany--Japan $5.2\times$, Argentina--Netherlands
$5.7\times$); the smallest single-team cell is Brazil at $1.4\times$, a perennially high-traffic article
with little headroom rather than a weak response. The tournament article, already elevated throughout the
event, moves less. The attention shock the rest of the paper conditions on is present at the hour of play,
not merely on the calendar day, so the market nulls below are a non-response to a shock that is real at
their own resolution.

\section{The aggregate global-attention channel}\label{sec:global}
$H_{\text{global}}$ asks whether the global attention shock depresses aggregate trading where no team maps to the marginal participant. It does not. Estimating Eq.~\eqref{eq:main} with log volume, the during-match change is $-2.7\%$ in crypto spot, $-3.9\%$ in crypto perpetuals, $-2.9\%$ in equity-index futures, $+7.2\%$ in commodity futures, and $+11.0\%$ in currency futures; pooled across all eleven instruments it is $+2.3\%$. None is significant, and the estimates are not consistently signed -- as compatible with a small increase as a small decrease, never the sharp decline distraction predicts. Table~\ref{tab:global} reports each cell with its interval and the decline it excludes. The bound is tightest in the deepest market: crypto spot, on 48 match-day clusters, rules out during-match volume declines beyond about $9\%$, and the pooled estimate rules out declines beyond about $6\%$. Equivalence tests against a $30\%$ margin reject an effect that large in every group. The bounds are not uniform, and we keep the gradient explicit rather than averaging it into one headline: crypto spot and the pooled estimate exclude declines beyond $9$ and $6\%$ and the $2.2$-million-minute quoted spread is pinned to $\pm1\%$, whereas the weekday-only futures cells are looser, ruling out only declines beyond about $15\%$ (equity-index futures) at a minimum detectable effect near $22\%$. The two futures cells with positive point estimates make this explicit. Their decline-excluded figure in Table~\ref{tab:global} is mechanically small --- FX futures excludes declines beyond only about $2\%$ --- but that is because the estimate sits above zero, not because the null is tight, since the largest decline still consistent with the data is read off the lower confidence bound. The cell's true precision is its interval width, summarized by the minimum detectable effect, and on that measure these futures cells remain the loose ones. The deep, continuously traded venues deliver precise nulls; the thinner cells deliver bounded ones. Figure~\ref{fig:bymarket} plots the same estimates against the $-30$ to $-50\%$ band the cash-equity literature reports; no interval reaches it.
...
Where every direct channel came up flat, one specification reproduces the prior literature's effect, for a reason that generalizes. Computing the mean pairwise return correlation across all available instruments in match versus control windows, comovement falls during matches by $-0.086$ ($p=0.005$), the attention-driven decline the theory predicts. What moves is the set of instruments open during matches, not their correlation. Because many matches fall on weekends, the match-window correlation set tilts toward crypto, a higher-correlation cluster, relative to the weekday control set, so the mean correlation shifts on composition alone. The bias is an identity, not a coincidence of our sample. The naive estimator is a difference of set-averages, $\widehat{\Delta}=\bar\rho_{S_m}-\bar\rho_{S_c}$, where $\bar\rho_S=\binom{|S|}{2}^{-1}\sum_{i<j\in S}\rho_{ij}$ averages the pairwise correlations over the instruments $S$ open in a window. When the open set differs across window types, $S_m\neq S_c$, this has nonzero expectation $\mathbb{E}\,\bar\rho_{S_m}-\mathbb{E}\,\bar\rho_{S_c}$ even with every pairwise $\rho_{ij}$ held \emph{identical} across windows, because the two averages run over different pairs; the weekend closures make $S_m$ crypto-heavy, a high-$\rho$ cluster, so $\widehat{\Delta}<0$ by construction. Conditioning on the instrument set, or adding an instrument-count control, equalizes $S_m$ and $S_c$ and annihilates the bias term --- which is precisely what the two checks that follow do.
...
\section{Country and cross-sectional heterogeneity}\label{sec:country}
The pooled home-market tests average over nations and results; five sharper cuts ask whether a reaction hides in a particular country, result type, or market culture. Each reorganizes the 1998--2022 index and 2002--2022 currency panels already built --- no new data --- and each is read through the same cluster-count discipline, with a wild-cluster restricted bootstrap on the cells below thirty match-day clusters (Table~\ref{tab:country}).

\emph{Brazil's own win.} We estimate every nation's own-win reaction on its own series, not Brazil's alone. Of the ten nations with enough World Cup wins to test ($\geq 8$), Brazil has the largest positive index reaction ($+74\bp$, rank first; the cross-nation median is $-7\bp$ and several are negative, so the result is not a noisy-everywhere artifact in which many nations look large), and the real is the one significant currency cell ($+55\bp$, $p=0.005$ --- though only Brazil and Uruguay have enough wins to test in the currency panel). Two cautions temper it. On its own index series Brazil is only marginal ($p=0.078$) and \emph{no} single index nation clears significance (0 of 10); the cells that do reach significance are the pooled cross-section contrast (IBOV $+91\bp$ over the cross-section, bootstrap $p=0.023$; real $+49\bp$, bootstrap $p=0.025$) and the currency, both of which borrow power from the panel. And the significance is partly carried by the 2002 championship --- seven of Brazil's wins, during the real's election-crisis swings --- since excluding 2002 the cross-section estimates stay positive but lose significance ($+76$ and $+39\bp$, bootstrap $p=0.07$ and $0.12$). We read it as a sign-robust, significance-fragile win-sentiment lead concentrated in one nation --- the optimistic mirror of the loser effect, the wrong direction for distraction, and a cell that does not survive the global correction below ($q=0.32$).

\emph{Loss aversion.} If losses bite harder than wins gratify, elimination should move markets more than advancement. Held in the same knockout context, the next session after a knockout win (advancing) and after a knockout loss (eliminated) are statistically identical and both null --- indices $+5\bp$ versus $-0\bp$, currencies $+22\bp$ versus $-11\bp$, none significant --- and the loss-times-elimination term adds nothing ($+10\bp$, $p=0.56$). No asymmetry appears, so no loss aversion at this resolution.

\emph{Developed versus emerging} is the size interaction of Section~\ref{sec:loser}, recomputed here and unchanged: the one nominal cell (emerging equities $+45\bp$ after a loss) is wrong-signed --- emerging markets fall \emph{less} --- and does not survive the bootstrap ($p=0.047\to0.053$).

\emph{Host nations.} A nation playing its own tournament has unique home exposure, yet host-team results move the next session no more than away results (host-win and host-loss interactions both insignificant; the one nominal cell, a $-32\bp$ host-win currency reaction, dies under the bootstrap, $p=0.024\to0.063$). Host-tournament match-days do carry $-0.34\%$ lower intraday index volatility (bootstrap $p=0.033$), a bootstrap-robust but economically trivial cell, and one of twenty-six in this family.

\emph{Football culture.} Coding nations whose dominant domestic spectator sport is a competing major league (United States, Japan, South Korea, Australia, Canada, New Zealand) against the football-centric remainder, the result-by-culture interactions are null in both return and volatility (all $p>0.15$): markets in the football-mad economies react no more than the rest. The coding is a coarse author-assigned binary, so this cut is the softest of the five, but it points the same way.

Folding these into the global pool --- the twenty heterogeneity coefficients plus the twelve per-nation own-win cells, the full family rather than a chosen subset --- lifts it to 190 during-match and own-team tests, of which 24 are nominally significant against $9.5$ expected by chance and, as before, none survives a single Benjamini--Hochberg correction; the most significant of the new cells, Brazil's win, is one nation and partly one tournament.

\begin{table}[h]\centering\footnotesize
\caption{Country and cross-sectional cuts of the home-market panels (no new data). Returns in basis points, volatility in percent; $95\%$ CI date-clustered; bootstrap $p$ shown for cells under thirty match-day clusters (wild-cluster restricted, $B=4999$). Read the bootstrap $p$ and the cluster count, not the point estimate, on the thin cells.}\label{tab:country}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{l>{\raggedright\arraybackslash}p{4.2cm}rrrr}\toprule
Cut & Headline cell & Estimate & 95\% CI & Clust. & $p$ (asy/boot) \\
\midrule
Brazil win & IBOV, own win vs cross-section & $+91\bp$ & $[18,164]$ & 27 & $.015/.023$ \\
 & real, own win vs cross-section & $+49\bp$ & $[9,88]$ & 23 & $.016/.025$ \\
 & \quad real, excluding 2002 & $+39\bp$ & $[-6,83]$ & 16 & $.087/.12$ \\
Loss aversion & eliminate (KO loss), idx & $-0.1\bp$ & $[-39,38]$ & 40 & $.996/.-$ \\
 & advance (KO win), idx & $+4.7\bp$ & $[-32,41]$ & 43 & $.80/-$ \\
Dev vs emerging & loss $\times$ emerging, idx & $+45\bp$ & $[1,89]$ & 28 & $.047/.053$ \\
Host & match-day vol $\times$ host, idx & $-0.34\%$ & $[-0.63,-0.04]$ & 24 & $.025/.033$ \\
 & host $\times$ win (return), ccy & $-32\bp$ & $[-61,-4]$ & 23 & $.024/.063$ \\
Football culture & win $\times$ football-centric, idx & $+65\bp$ & $[-24,153]$ & 104 & $.15/-$ \\
\bottomrule
\end{tabular}
\end{table}
...
findings. One caveat scopes the recovery rates: Table~\ref{tab:inject} injects a homogeneous shift on every
match-minute, so it speaks to a \emph{uniform} effect, whereas the Ehrmann--Jansen effect is largest ``at the
most attentive moments.'' We therefore re-run the injection with the same \emph{average} decline placed on
only a fraction $\phi$ of each match-day's in-play minutes, at per-minute magnitude $\delta/\phi$, so the
concentration leaves more within-cluster residual and inflates the date-clustered standard error.
Concentrating a $-30\%$ average into a quarter ($\phi=0.25$) and then a tenth ($\phi=0.10$) of the window
leaves the well-powered cells at power $1.00$ with the recovered coefficient still at the injected average
(at $\phi=0.10$: crypto spot $-31.9\%$, equity futures $-32.5\%$, FX futures $-32.5\%$, pooled own-team
$-30.5\%$), and the $-45\%$ average behaves identically. At these cluster counts the detection headroom is
large enough that concentration does not degrade power, so the nulls are informative against a concentrated
literature-magnitude effect, not only a uniform one. The check holds the \emph{average} fixed; an effect
whose per-minute size matches the literature but whose \emph{average} over the in-play window is single-digit
is governed by each cell's minimum detectable effect rather than by this control.
...
$H_{\text{local}}$ admits a narrower reading. The distraction effect does not appear in any mapped venue we can reach, yet the venues that match Ehrmann--Jansen's resolution carry a marginal trader who is not the viewer, and the venue with the right trader is daily. We can bound the daily home response and we can say the effect does not show up in CME currency futures or US country ETFs, neither of which is traded by the distracted viewer. The original minute-level effect on a foreign cash exchange stays beyond reach, and our daily home-equity null is silent on an intraday dip that averages out over the session. The powered part of that mapped evidence is moreover essentially the euro --- the other own-currency cells are individually underpowered --- so it is best read as one clean currency plus a set of bounds rather than four independent confirmations. What the data license is generalization-bounding, not refutation: the effect, real in its own setting, does not travel to the global and derivative venues that now dominate volume, and its survival at the minute level on a home exchange is left open.
...
The full pull-and-analysis pipeline is public at \url{https://github.com/DaruFinance/worldcup-attention-markets}; market data is not redistributed (AlgoSeek is licensed) but the scripts document every source and build step. The repository pins its environment (\path{requirements.txt}), and \path{run_all.py} regenerates every figure and table in dependency order, skipping any stage whose licensed input is absent so that a non-holder still reproduces the schedule, attention, control, and free-data (Binance crypto and daily home-equity) results; the AlgoSeek-derived futures, equity, quote (TAQ) and options panels are licensed and not redistributed, with their parse scripts provided so a license-holder can rebuild them. The committed inputs are fixed by SHA-256 in \path{DATA_MANIFEST.sha256}. Every stochastic
...
\path{data/controls/} so the regressions reproduce bit-for-bit without a network call. A top-level \path{run_all.py} executes these stages in dependency order, skipping any whose licensed
input is absent and verifying the committed inputs against the manifest (\path{run_all.py --check}).
Core analyses:
the cross-market and aggregate-channel tests (\path{analysis/regress_crossmarket.py}, \path{regressions.py}), the
...
the five country and cross-sectional cuts of Section~\ref{sec:country} (\path{country_effects.py},
reusing those panel loaders and emitting \path{country_effects_summary.csv} and the per-cut files),
...
(\path{injection_power.py} for the uniform shift and \path{injection_concentrated.py} for the
concentrated-shock variant, emitting \path{injection_power.csv} and \path{injection_concentrated.csv}), the loser-effect calendar placebo of
...
reconstruction of ten matches across both tournaments (\path{attention_final.py}, emitting
...
The pre-registered primary family (during-match volume, trade count, absolute return, and high--low range
across the five market groups, the \texttt{activity:} rows of \path{master_fdr.csv}) holds twenty
coefficients, none nominally significant (smallest $p=0.12$), so the family-restricted Benjamini--Hochberg
correction is vacuous. The full multiple-testing pool (\path{master_fdr.csv}) holds all 190 during-match and
own-team coefficients across activity, liquidity, options, efficiency, comovement, intensity, goal windows,
sentiment, domestic, home-equity, home-currency, breadth, and country-heterogeneity families; 24 are
nominally significant --- all outside the primary family --- none survives the global correction, and the
smallest-$p$ cells are the mechanism-diagnosed artifacts named in Sections~\ref{sec:robust} and~\ref{sec:country}. Equivalence bounds for every primary cell are in \path{equivalence_tost.csv} and
